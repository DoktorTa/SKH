...
class IFSWork(metaclass=ABCMeta):
    ...
    def read(self, dir_now: str, num_in_dir: int, count: int, pointer: int) -> (bytes, int, int):
        """
            ...
            Третий: количество блоков на котороенеобходимо провести чтение с нулем в текушей точке указателя.
            Четвертый: текущая позиция указателя на последний прочинанный блок для этого номера именно в этом листе элементов.
    ...
    def cd(self, dir_now: str, num_in_dir: int) -> (list, int):
        """
            ...
            Первый аргумент ответа: новый лист элементов для номера в старом листе в случае если он директория.
