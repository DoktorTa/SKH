...
class ELFWork(IExecutableFile):
    """
        Класс выполняет роль контроллера доступа, интерфейс не является
         обязательным, ввиду отсуцтвия у меня понимани структуры других
          исполняемых файлов.
    """
    __elf: ELFReader
    ...
    def __init__(self, file):
        data = ELFData()
        # TODO: Я совершенно не понимаю как сделать лучше,
        #  передовать этот ^^^ обьект в конструктор или создавать прямо в нем,
        #  в первом случае сформируется зависимость которой хотелось бы
        #  избежать, во втором непонятно можно ли так делать.
